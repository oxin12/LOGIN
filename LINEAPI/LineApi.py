...
class LINE:
  ...
  def __init__(self):
    ...
    self.Talk.set_Headers('X-Line-Access',akun['token1'])
  ...
  def login(self, mail=None, passwd=None, cert=None, token=None, qr=False, callback=None):
    ...
    self.obs_token = self.Talk.client.acquireEncryptedAccessToken(1)
    self.cert = self.Talk.cert
    self._headers = {
              'X-Line-Application': 'DESKTOPWIN\t8.3.0RFU-BOT\t18.99', 
              'X-Line-Access': self.authToken, 
              'User-Agent': 'Line/8.3.3 iPad4,1 9.0.2'
   }

    self.Poll = Poll(self.authToken)
    self.channel = channel.Channel(self.authToken,self.obs_token)
    self.channel.login()
  ...
  def upload_pp(self, path):
        return requests.post(url="https://obs.line-apps.com/os/p/%s" % self.mid,
  ...
  def changePP(self, to_, id_,from_):
      ...
      try:
          self.sendText(to_,"Change image success!")
      except Exception as e:
          raise e
